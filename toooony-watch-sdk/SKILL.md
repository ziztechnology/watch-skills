...
description: Integration, implementation, testing, and review guidance for the Toooony watch face frontend SDK @ziztechnology/dial-library, covering basic sensors, Bluetooth Now Playing snapshots, the SDK 0.1.0+ seven-state driving model, schema v2 driving expression configuration and legacy migration, one-time analysis, expression playback, media URLs, page lifecycle, Sticker carousels, in-car multimedia controls, and the browser testing Runtime exported from the /testing subpath. Use when writing, completing, modifying, fixing, refactoring, testing, or reviewing watch face frontend code that uses this SDK, including Runtime Bridge compatibility, Bluetooth media-session metadata and capability handling, driving-status migration, local provider and lifecycle simulation, multimedia command testing, player resource cleanup, debugging, and troubleshooting.
---
...
5. Define explicit handling paths for an unavailable Runtime, an unavailable DeviceAgent channel, failed snapshot reads, inactive Bluetooth media sessions, insufficient permissions, missing sensor samples, invalid configuration, media loading failures, and page suspension.
...
- Read [testing.md](references/testing.md) when writing browser-based SDK tests, installing the simulated Runtime, supplying sensor or Bluetooth Now Playing snapshots, injecting driving expression configuration, dispatching multimedia or Runtime lifecycle events, handling test isolation, or configuring local development and HMR cleanup.

# Implementation Requirements

- Use sensors, Bluetooth Now Playing, and in-car multimedia capabilities only through `@ziztechnology/dial-library`. Do not call internal methods injected by the Runtime into `window` directly.
